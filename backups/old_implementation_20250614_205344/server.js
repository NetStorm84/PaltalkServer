    ...
    switch (packetType) {
        case PACKET_TYPES.ADD_PAL:
            ...
            if (!thisUser.buddies.includes(userToAdd.uid)) {
                ...
                } else if (userToAdd.nickname === 'Paltalk' || userToAdd.uid === 1000001) {
                    // Special case: Paltalk user should always appear online
                    console.log(`Sending Paltalk online status to ${thisUser.nickname} after buddy addition`);
                    sendPacket(socket, PACKET_TYPES.STATUS_CHANGE, Buffer.from((userIdHex + '0000001E'), 'hex'));
                }
...
async function handleLogin(socket, payload) {
    ...
    buddies.forEach(buddy => {
        ...
        if (buddySocket) {
            // Buddy is currently online, show them as online
            ...
            // Special case: Paltalk user should always appear online
            console.log(`Sending Paltalk online status to ${user.nickname} (${user.uid})`);
            sendPacket(socket, PACKET_TYPES.STATUS_CHANGE, Buffer.from(helper.conversions.decToHex(buddy.uid) + '0000001E', 'hex'));
        }
    });

    // Ensure Paltalk user always appears online to users who have them in buddy list
    ensurePaltalkOnlineStatus(currentSockets.get(socket.id), buddies);
...
function ensurePaltalkOnlineStatus(userSocket, userBuddies) {
    // Check if this user has Paltalk in their buddy list
    const paltalkBuddy = userBuddies.find(buddy => 
        buddy.nickname === 'Paltalk' || buddy.uid === 1000001
    );
    
    if (paltalkBuddy) {
        console.log(`Ensuring Paltalk appears online for user ${userSocket.user.nickname}`);
        // Send online status for Paltalk user (UID: 1000001, Status: 0x1E = Online)
        sendPacket(userSocket.socket, PACKET_TYPES.STATUS_CHANGE, 
            Buffer.from(helper.conversions.decToHex(1000001) + '0000001E', 'hex'));
    }
}